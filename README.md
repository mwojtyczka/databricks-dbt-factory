...
- `--select` (type: str, optional, default: None): dbt-style selector to scope the generated tasks to a subset of the manifest — useful for monorepo projects that generate one job per domain without pre-filtering the manifest. Supports a space-separated union of `tag:<tag>`, `path:<dir>`, `fqn:<a.b.c>`, and bare name/fqn selectors, each optionally wrapped in the `+`/`@` graph operators (e.g. `--select "tag:daily"`, `--select "+my_model"`, `--select "staging.orders+"`). Multiple selectors can be combined in one space-separated string and are **unioned** — a node is kept if it matches any of them (e.g. `--select "tag:daily +my_model path:models/staging"` keeps everything tagged `daily`, plus `my_model` and its ancestors, plus everything under `models/staging/`), matching `dbt ls --select "a b"`. Note that `path:` is matched as a prefix of the node's full file path (e.g. `path:models/staging`, not `path:staging`). Resolved from the manifest alone with no dbt invocation, so the full dbt selector grammar (set intersections with commas like `tag:daily,path:staging`, `state:`, `config:`, …) is **not** supported. Deselected upstream models are pruned from `depends_on`, mirroring `dbt build --select`.
